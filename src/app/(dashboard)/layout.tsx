...
export default function DashboardLayout({
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col justify-between p-6 bg-white border-r border-zinc-200/80 shadow-xs shadow-zinc-150/40">
      <div className="space-y-6">
        ...
        <Link href="/" className="flex items-center gap-3 px-2 transition-transform hover:scale-102">
          ...
          <div>
            <span className="font-extrabold text-xl tracking-tight bg-gradient-to-r from-zinc-900 to-zinc-600 bg-clip-text text-transparent">Voicey</span>
            <p className="text-[10px] text-zinc-400 font-bold tracking-widest uppercase">Self-Hosted TTS</p>
          </div>
        </Link>

        {/* Plan Status Card */}
        {isSignedIn && (
          <div className="p-4 rounded-xl bg-zinc-50 border border-zinc-200/60 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-zinc-500 font-medium">Current Plan</span>
              {planLoading ? (
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-300 ${
                  isActive
                    ? "bg-violet-600/10 text-violet-700 border-l-2 border-violet-600 pl-2.5 shadow-xs shadow-violet-500/5"
                    : "text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100/70"
                } hover:translate-x-0.5`}
              >
                <Icon className={`h-4.5 w-4.5 transition-transform duration-300 ${isActive ? "text-violet-600 scale-105" : "text-zinc-400 group-hover:text-zinc-600"}`} />
                {item.name}
              </Link>
            );
          })}
      ...
      {/* User Actions */}
      <div className="flex flex-col gap-3 border-t border-zinc-200/80 pt-4 px-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <UserButton 
              appearance={{
                elements: {
                  userButtonAvatarBox: "h-9 w-9 rounded-lg border border-zinc-200 shadow-xs",
                }
              }}
            />
            <div 
            ...
            >
              <span className="text-xs font-bold text-zinc-800">My Account</span>
              <span className="text-[10px] text-zinc-500 font-medium">Settings & billing</span>
            </div>
          </div>
          ...
          <Button
            ...
            className="text-zinc-400 hover:text-rose-600 hover:bg-rose-500/10 h-8.5 w-8.5 rounded-lg transition-all"
            title="Sign Out"
          >
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-tr from-violet-50/40 via-zinc-50 to-indigo-50/40 text-zinc-800 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
        {/* Mobile Navbar */}
        <header className="flex md:hidden items-center justify-between px-6 py-4 bg-white border-b border-zinc-200/80 shadow-xs">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-lg text-zinc-900 tracking-tight">Voicey</span>
          </Link>
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="p-1 rounded-lg text-zinc-500 hover:text-zinc-800 transition-colors focus:outline-none"
          >
